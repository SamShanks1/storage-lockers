...
Padlock.Close = function() {
    ...
    $.post('https://shanks-storagelockers/PadLockClose');
}
...
Keypad.Close = function(data) {
    ...
    $.post('https://shanks-storagelockers/PadLockClose');
    if (data.error != null) {
        $.post('https://shanks-storagelockers/CombinationFail');
    }
}
...
function submitForm(e) {
    $("#keypad").css("display", "none");
    $.post("https://shanks-storagelockers/UseCombination", JSON.stringify({
        type: type,
        combination: e.value,
    }));
};
...
function outOfPins() {
    gameOver = true;
    $.post('https://shanks-storagelockers/fail');
    setTimeout(function(){
        reset()
    }, 250)
}
...
function unlock() {
    gameOver = true;
    $.post('https://shanks-storagelockers/success');
    solveDeg = (Math.random() * 180) - 90
    solvePadding = 4
    maxDistFromSolve = 45
    pinRot = 1
